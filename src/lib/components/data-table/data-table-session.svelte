<script lang="ts">
	...
	import DataTableRfidFilter from '$lib/components/data-table/column-filters/rfid-filter.svelte';
	...
	import RfidIdentifier from './fields/rfid-identifier.svelte';
	...
	const columns = table.createColumns([
		...
		table.column({
			accessor: (item) => item.chargingSession.rfidTimestamp,
			id: 'rfidTimestamp',
		...
		table.column({
			accessor: (item) => [item.employee, item.rfidDescription],
			id: 'rfidIdentifier',
			header: 'RFID identifikátor',
			cell: ({ value }) => {
				return createRender(RfidIdentifier, {
					employee: value[0],
					rfidDescription: value[1]
				});
			},
			plugins: {
				sort: {
	...
	const hiddenColumns = ['id', 'chargerId', 'controllerId', 'rfidTimestamp'];
...
<div class="flex flex-col gap-4 w-full overflow-x-auto">
	<div class="flex justify-between items-end">
		<div class="flex items-center gap-2">
			<DataTableStateFilter {pluginStates} />
			<DataTableRfidFilter {pluginStates} />
