<script lang="ts">
	...
	import FilterButton from './data-table-column-filter/filter-button.svelte';
...
<Popover.Root>
	<Popover.Trigger>
		<FilterButton {pluginStates} valueName="state" preview={selectedValue.label}>Stav</FilterButton>
	</Popover.Trigger>
	<Popover.Content align="start" class="flex flex-col gap-4 w-full max-w-[15rem] py-2 px-3 mt-1">
		...
		<Select.Root bind:selected={selectedValue}>
			...
			>
				<Select.Value
					class="text-accent-foreground font-medium first:absolute first:left-1/2 first:-translate-x-1/2 first:w-full"
				/>
			</Select.Trigger>
			<Select.Content>
				<Select.Group
